...
（原「SplitPane leaf renders Pane compound component」全面修訂）
...
### Requirement: zoom 以 hidden 屬性隱藏其他 pane

zoom／mobile 的行為改為 solo rendering（PaneSplit 只渲染含目標的一側，無 percentage wrapper 與 divider）——見 `specs/pane-tree/spec.md` 的「Zoom and mobile solo rendering happens at PaneSplit」。
